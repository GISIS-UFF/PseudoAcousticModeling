...
public:
    ...
    float calculateGradientOntheFly();
    float calculateGradientCheckpoint();
    float calculateGradient(const std::string& parameter, float* gradient);
    float calculateMultiparameterGradient(bool update_eps, bool update_delta, bool update_theta, float* gradient_vp, float* gradient_eps, float* gradient_delta, float* gradient_theta);
    ...
    float armijolinesearch(const std::string& parameter, const float* model0, const float* grad0, const float* p, float X0, bool empty, float scale);
    float linesearch(const std::string& parameter, const float* model0, const float* grad0, const float* p, float X0, float& X_new, float* grad_new, bool empty, float scale);
    float zoom(const std::string& parameter, const float* model0, const float* grad0, const float* p, float X0, double gp0, float alpha_lo, float alpha_hi, float X_lo_initial, float& X_new, float* grad_new, float scale);
    float linesearchMultiparameter(const float* vp, const float* epsilon, const float* delta, const float* theta, const float* p_vp, const float* p_eps, const float* p_delta, const float* p_theta, const float* g_vp, const float* g_eps, const float* g_delta, const float* g_theta, float beta_vp, float beta_eps, float beta_delta, float beta_theta, float scale_vp, float scale_eps, float scale_delta, float scale_theta, bool update_eps, bool update_delta, bool update_theta, float X0);
