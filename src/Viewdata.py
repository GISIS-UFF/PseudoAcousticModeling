...
class plotting:
    ...
    def movieSnapshot(self,keyword_snap,path_model,backward=False,interval=200,savegif=False):
        perc = 98
        snap_files = []
        ...
        if backward:
            ...
            propagation_name = "Backward"
        else:
            snap_files.sort(key=lambda x: (x[0], x[1]))
            propagation_name = "Forward"
        ...
        amp = 0.0
        sample_step = max(1, len(snap_files) // 20)

        for _, _, filename in snap_files[::sample_step]:
        ...
        title = ax.set_title(f"{propagation_name} | "f"Shot {first_shot} | "f"Frame {first_frame} | "f"Time {first_frame * self.pmt.dt:.4f} s")
        ax.set_xlabel("Distance (m)")
        ax.set_ylabel("Depth (m)")
        ...
        def update(i):
            ...
            title.set_text(f"{propagation_name} | "f"Shot {shot} | "f"Frame {frame} | "f"Time {frame * self.pmt.dt:.4f} s")
            return im, title

        ani = FuncAnimation(fig,update,frames=len(snap_files),interval=interval,blit=True)
        if savegif:
            direction = "backward" if backward else "forward"
            gif_path = os.path.join(self.pmt.snapshotFolder,f"snapshots_{direction}.gif")
